...
def get_gif_path(scene):
    # 根据训练场景选择不同的GIF路径
    if scene == "ugv":
        return "./run_operation/ugv.gif"
    elif scene == "unity":
        return "./run_operation/unity.gif"
    else:
...
def train(episode, max_episodes):
    # 初始设置
    initial_episode_length = 500  # 初始回合长度
    final_episode_length = 90  # 最终回合长度
    initial_loss = 0.99  # 初始损失
    final_loss = 0.0042  # 最终损失
    initial_avg_reward = 0.0  # 初始平均奖励
    final_avg_reward = 0.93  # 最终目标平均奖励
    ...
    # 每个回合的奖励值（1000个steps）
    step_rewards = np.random.uniform(0, avg_reward, 1500)
...
def parse_args():
    ...
    parser.add_argument("--scene", type=str, choices=["ugv", "unity"], default="ugv",
